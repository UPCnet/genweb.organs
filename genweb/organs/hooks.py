...
def acordAdded(content, event):
    """ Acord added handler """
    addEntryLog(content.__parent__, None, _(u'Created acord ROB'), str(content.Title()))


@grok.subscribe(IActa, IObjectAddedEvent)
def actaAdded(content, event):
    """ Acta added handler """
    addEntryLog(content.__parent__, None, _(u'Created acta ROB'), str(content.Title()))


@grok.subscribe(IPunt, IObjectAddedEvent)
def puntAdded(content, event):
    """ Punt added handler """
    addEntryLog(content.__parent__, None, _(u'Created punt ROB'), str(content.Title()))


@grok.subscribe(ISubpunt, IObjectAddedEvent)
def subpuntAdded(content, event):
    """ Punt added handler """
    addEntryLog(content.aq_parent.aq_parent, None, _(u'Created subpunt ROB'), str(content.Title()))
...
def actaDeleted(content, event):
    """ Acta delete handler
    """
    addEntryLog(content.__parent__, None, _(u'Deleted acta ROB'), content.absolute_url())
...
def actaModified(content, event):
    """ Acta modify handler """
    addEntryLog(content.__parent__, None, _(u'Modified acta ROB'), content.absolute_url())


@grok.subscribe(IPunt, IObjectModifiedEvent)
def puntModified(content, event):
    """ Punt modify handler """
    addEntryLog(content.__parent__, None, _(u'Modified punt ROB'), content.absolute_url())


@grok.subscribe(ISubpunt, IObjectModifiedEvent)
def subpuntModified(content, event):
    """ Subpunt modify handler """
    addEntryLog(content.aq_parent.aq_parent, None, _(u'Modified subpunt ROB'), content.absolute_url())
