...
Change session state to convened
  ...
  Click Element  workflow-transition-convocant
  Confirm Action
  Click Button  form-buttons-send

We make checks when the session is in convened
  Change session state to convened
  Log  \n*** Sessió convocada ***  console=True
  We make the checks for OG secretary when the session is in convened
  We make the checks for OG editor when the session is in convened
  We make the checks for OG member when the session is in convened
  We make the checks for OG affected when the session is in convened
  We make the checks for not role user when the session is in convened
  We make the checks for not validated user when the session is in convened
